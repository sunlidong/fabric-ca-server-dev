...
`Fabric CA repository <https://caserver>`__ for more
information.
...
::
    ...
    <gopath>/src/caserver; git pull --ff-only
    ...
    package caserver/cmd/fabric-ca-client: exit status 1
...
Navigate to `$GOPATH/src/caserver/docker/server`
...
`swagger-fabric-ca.json <https://caserver/blob/master/swagger/swagger-fabric-ca.json>`_.
...
You can refer to the `handleIdemixEnroll` function in https://caserver/blob/master/lib/client.go for reference implementation
